...
function ReviewCard({ r }: { r: Review }) {
  return (
    <div className="shrink-0 w-[320px] sm:w-[380px] mr-5">
      <div className="bg-white rounded-2xl ring-1 ring-black/5 p-6 shadow-md h-full flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-1 text-accent mb-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <Wand key={i} className="size-4 shrink-0" />
            ))}
          </div>
          <p className="relative text-base sm:text-sm text-foreground/85 leading-relaxed mb-5 text-pretty before:absolute before:inline before:-translate-x-full before:content-['\201C'] after:inline after:content-['\201D']">{r.text}</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="size-10 rounded-full bg-primary text-white grid place-items-center font-heading font-semibold outline-1 -outline-offset-1 outline-black/5">
            {r.name.slice(0, 1)}
          </div>
          <div>
            <p className="font-heading font-semibold uppercase text-sm text-primary">
              {r.name}
            </p>
